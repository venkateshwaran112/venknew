...
//		opt.addArguments("--incognito");                //newly added me
//		opt.addArguments("--start-maximised");           // newly added me
	...
	public static Properties readPropertyFile() {
		try {
			...
			FileInputStream propFile = new FileInputStream(
					"src\\test\\resources\\com\\property\\user.properties");
			prop = new Properties();
			prop.load(propFile);
...
//	}
	...
	public static void getaccUrl() {
		try {
			driver.get(prop.getProperty("accurl"));
			Thread.sleep(5000);
			driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(20));
			driver.manage().window().maximize();
			driver.manage().deleteAllCookies();
